...
module.exports = async callback => {
    ...
    while (index < length) {
        ...
        if (fs.existsSync('dnd-metadata/' + characterMetadata['name'].toLowerCase().replace(/\s/g, '-') + '.json')) {
            console.log('test')
            continue
        }
        ...
        filename = 'dnd-metadata/' + number + '-' + characterMetadata['name'].toLowerCase().replace(/\s/g, '-')
